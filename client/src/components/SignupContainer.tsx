  ...
  const handleSignup = async () => {
    ...
    if (
      ...
      if (confirmPasswordField.value !== passwordField.value) {
        ...
        hasError = true;
      } else if (confirmPasswordField.value.length < 8 ||  passwordField.value.length<8) {
        passwordField.setError('Password must be at least 8 character long');
        confirmPasswordField.setError('Password must be at least 8 character long');
    ...
    <div className="w-[536px] h-[100vh] absolute right-0 px-[48px] gap-[44px] flex flex-col justify-center items-start roudend-[8px]  bg-[#F5F5F5]">
      ...
      <div className='w-[440px] gap-[40px] min-h-[649px] flex flex-col justify-center items-center'>
        ...
        <div className='w-[100%] min-h-[483px] flex flex-col gap-[24px] '>
          ...
          <form className='w-[100%] min-h-[336px] flex flex-col gap-[16px]'>
            <div className='w-[100%] flex flex-col gap-[8px] justify-between min-h-[70px]'>
              ...
              placeholder='Input full name'></input>
              {fullnameField.error && (<p className='text-[rgba(144,11,9,1)] leading-[1.4] min-h-[20px] text-sm'>
                {fullnameField.error}
              </p>)}
            </div>
            <div className='w-[100%] flex flex-col gap-[8px] justify-between min-h-[70px]'>
              ...
              ></input>
              {emailField.error && (<p className='text-[rgba(144,11,9,1)] leading-[1.4] min-h-[20px] text-sm'>
                {emailField.error}
              </p>)}
            </div>
            <div className='w-[100%] flex flex-col  gap-[8px] justify-between min-h-[70px]'>
              ...
              {passwordField.error && (<p className='text-[rgba(144,11,9,1)] leading-[1.4] min-h-[20px] text-sm'>
                {passwordField.error}
              </p>)}
            </div>
            <div className='w-[100%] flex flex-col gap-[8px] justify-between min-h-[70px]'>
              ...
              {confirmPasswordField.error && (<p className='text-[rgba(144,11,9,1)] leading-[1.4] min-h-[20px] text-sm'>
          ...
          <div className='w-[416px] min-h-[17px] flex flex-col items-start gap-2 mt-2'>
            ...
            {checkerror.trim() && (<p className='text-[rgba(144,11,9,1)] leading-[1.4] min-h-[20px] text-sm'>
                {checkerror.trim()}
              </p>) }
          </div>
          <button 
        ...
        <div className='w-[100%] h-[38px] flex flex-row justify-center items-center gap-[8px]'>
          ...
          <button onClick={() => navigate('/auth/login')} className='h-[38px] py-[8px] px-[0] font-bold hover:underline cursor-pointer'>Sign in</button>
        </div>
      </div>
    </div>
  )
