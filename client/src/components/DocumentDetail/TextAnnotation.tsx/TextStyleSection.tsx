...
import { setTextAnnotation } from "../../../store/documentDetailSlice/textAnnotationSlice";
  ...
  return (
    <div className="w-full mb-4 flex flex-col gap-1">
      <h3 className="text-sm font-semibold text-gray-700 mb-2">
        {t("docDetail.textSyle")}
      </h3>
      <div className="w-full flex gap-4">
        <div className="w-[178px] px-2 border border-gray-300 bg-white rounded-sm">
          <select
            value={textAnnotation.textFont}
            onChange={(e) => dispatch(setTextAnnotation({textFont:e.target.value}))}
        ...
        <div className="flex-1 px-2 border border-gray-300 bg-white rounded-sm min-w-0">
          <select
            value={textAnnotation.textSize}
            onChange={(e) => dispatch(setTextAnnotation({textSize:e.target.value}))}
            className="p-[3px] w-full focus:outline-none focus:ring-0 focus:border-transparent"
          >
            {textSizes.map((size) => (
      ...
      <div className="flex w-full justify-between gap-2">
        {textcolors.map((color, index) => (
          <button
            key={index}
            onClick={() => dispatch(setTextAnnotation({textColor:color}))}
          ...
          ${color === textAnnotation.textColor ? "ring-2 ring-black ring-offset-1" : ""}
            `}
            />
          </button>
        ))}
